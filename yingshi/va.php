    ...
    <div class="am-u-md-3">
        ...
        <select placeholder="类型" name="cat" data-am-selected="{btnWidth: '100%', maxHeight: 225}" 
         class="filter-change-listen" id="movie-cat">
			...
			preg_match_all('/https:\/\/www.360kan.com\/zongyi\/list\?act=all&area=all&cat=(.*?)" target="_self" monitor-shortpv-c="tab_类型:_(.*?)"/si',$yuanma,$leixing); 			
    ...
    <div class="am-u-md-3">
        ...
        <select placeholder="地区" name="area" data-am-selected="{btnWidth: '100%', maxHeight: 225}" 
         class="filter-change-listen" id="movie-area">
            <option selected value=""></option>
            <option value="all">全部</option><?php
			preg_match_all('/https:\/\/www.360kan.com\/zongyi\/list\?cat=all&act=all&area=(.*?)" target="_self" monitor-shortpv-c="tab_地区:_(.*?)"/si',$yuanma,$leixing); 
			foreach ($leixing[2] as $n=>$lei){
				echo '
			<option value="'.$leixing[1][$n].'">'.$lei.'</option>';
			} ?>
